...
export const buildMemoryPrompt = ({ memory, events }) => {
  ...
  return [
    ...
    '- Prefer capturing concrete continuity hooks: items acquired/lost (keys, tools), named characters met/helped, routes/shortcuts revealed, current threats, and durable state flags.',
    '- If events mention an item/key/tool, add a fact with topic "item" and entity set to the item name.',
    '- If events mention a named character interaction, add a fact with topic "character" and entity set to the character name.',
    '- You MAY infer durable state from ATTRS in events. If ATTRS contains boolean flags like hasX=true, treat them as item possession and add item facts (e.g., hasSpadeKey=true => entity "Spade Key").',
    '- Avoid generic location facts like "Player is at the police station" unless there are no better facts. Prefer specific places (e.g., "Records Corridor").',
